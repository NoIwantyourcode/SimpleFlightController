# Introduction
Hi!,

Welcome to my simple filght controller repository

This is a flight controller as the name suggests that is specifically designed to pilot remote controlled aircraft and provides accurate data to make flying your plane easier!,
some features I plan for to be included in the firmware are
       - 3 axis stabilization
       - altitude management

This is not a comprehensize list of features I may include and the actual firmware may have a lot more


Here are a few images of the Flight controller Schematics, PCB view and Rendered PCB view


# Schematics

<img width="1499" height="1063" alt="Screenshot 2026-01-07 170808" src="https://github.com/user-attachments/assets/6a7e3cd3-3884-42f1-b248-0b2f4b2a6139" />

# PCB view

<img width="790" height="858" alt="Screenshot 2026-01-07 170818" src="https://github.com/user-attachments/assets/fb0c22fe-35da-4bb0-9756-fb371b1348ea" />

# Rendered PCB View

<img width="812" height="859" alt="Screenshot 2026-01-07 170835" src="https://github.com/user-attachments/assets/350d68de-e507-4566-a1d3-9598cfb580dd" />
<img width="839" height="914" alt="Screenshot 2026-01-07 172151" src="https://github.com/user-attachments/assets/a27f8c43-7d36-453b-a670-48b9be627962" />

# Display case

<img width="2879" height="1802" alt="Screenshot 2026-01-07 182156" src="https://github.com/user-attachments/assets/70af9535-5e28-40b0-8b3e-f141e1af7bb6" />

NOTES TO REVIEWER:

I have had to substitute a few of components due to part shortfalls like the ICM-45686 had to substituted with the ICM-45605
